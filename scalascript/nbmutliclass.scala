...
}).randomSplit(Array(0.7, 0.25, 0.05))
...
val predictionAndLabels = eval.map(p => (model.predict(p.features), p.label))
predictionAndLabels.take(20).foreach(println)

val pl = eval.map(p => (model.predictProbabilities(p.features), p.label))
pl.take(20).map(x => x._2 + ":\t" + x._1.toArray.map(y => f"$y%1.2f").mkString(",")).foreach(println)